        ...
        case true:
            if maxMenus > 0 {
                for i in 0...maxMenus - 1 {
                    ...
                    kakaoShareInfo["price\(i+1)"] =
                        restaurant.menus[i].price != 0 ? "\(restaurant.menus[i].price)원" : "-"
                }
            }
            for i in maxMenus...5 {
    ...
    func shareKakao(restaurant: KakaoShareRestaurantModel, selectedDateString: String) {
        ...
        if ShareApi.isKakaoTalkSharingAvailable() {
            ...
            if let sharingResult = ShareApi.shared.makeCustomUrl(templateId: templateId, templateArgs: kakaoShareInfo) {
                ...
                let error = NSError(
                    domain: "CustomURL", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to create custom URL"])
                print(error)
            }
        }
    }
