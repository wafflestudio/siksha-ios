...
extension Repository: CommunityRepositoryProtocol {
    ...
    func submitPost(
        boardId: Int, title: String, content: String, images: [Data], anonymous: Bool
    ) -> AnyPublisher<SubmitPostResponse, AppError> {
        let endpoint = SikshaAPI.submitPost(
            boardId: boardId, title: title, content: content, images: images, anonymous: anonymous)
        return self.networkModule.request(endpoint: endpoint)
    }

    func editPost(
        postId: Int, boardId: Int, title: String, content: String, images: [Data], anonymous: Bool
    ) -> AnyPublisher<SubmitPostResponse, AppError> {
        let endpoint = SikshaAPI.editPost(
            postId: postId, boardId: boardId, title: title, content: content, images: images, anonymous: anonymous)
        return self.networkModule.request(endpoint: endpoint)
    }
