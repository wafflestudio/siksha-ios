...
struct DistanceSliderView: View {
    ...
    private let sliderBackgroundColor: Color = Color.elementControl
...
struct SliderValueIndicator: View {
    ...
    private let backgroundColor: Color = Color.elementTooltip
...
struct BubblePointer: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        path.addArc(
            tangent1End: CGPoint(x: rect.minX + cornerRadius, y: topY),
            tangent2End: CGPoint(x: rect.minX, y: topY),
            radius: cornerRadius)
        path.addLine(to: CGPoint(x: rect.maxX, y: topY))
        path.addArc(
            tangent1End: CGPoint(x: rect.maxX - cornerRadius, y: topY),
        ...
        path.addArc(
            tangent1End: CGPoint(x: rect.midX, y: bottomY),
            tangent2End: CGPoint(x: rect.midX - cornerRadius, y: bottomY - cornerRadius),
            radius: cornerRadius)
